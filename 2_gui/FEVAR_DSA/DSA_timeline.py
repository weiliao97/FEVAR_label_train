...
# ============ user inputs ============
weights_name_f = 'cnn_encoder_acc_0.90_epoch148.pth'
weights_name_dsa = 'cnn_encoder_acc_0.767_epoch90.pth'
# validation set data directory 
datapath = 'C:/Users/320190618/Documents/code_compile/2_gui/FEVAR_DSA'
datapath_d = 'C:/Users/320190618/Documents/code_compile/2_gui/FEVAR_DSA/DSA'
...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.label_info_1 = customtkinter.CTkLabel(master=self.frame_right,
                                                    text="Analyzing BIDMC 20211104_Pat5005 PMEG" ,
                                                    height=30,
                                                    corner_radius = 5, 
        ...
        self.legend_t = []
        for i in range(5):
            self.legend_l.append(self.legend_canvas.create_line(190*0.05, 204*(0.15+text_gap[i]), 190*0.5,  204*(0.15+text_gap[i]),  width=5, fill = colormap[i],))
            self.legend_t.append(self.legend_canvas.create_text(190*0.55, 204*(0.15+text_gap[i]), text = label_dict[i], anchor=W, font=('Helvetica','10','bold')))
        
        text_gap = [0.78/4*i for i in range(1, 5)]
        stat_dict = ['Navigation: 7.6 min', 'Unsheath: 121.8 min', 'Cannulation: 76.6 min',\
                     'Final deploy: 37.6 min']
